...
<html lang="en">
  ...
  <body class="h-screen m-0 p-0 overflow-hidden relative">
    ...
    <div class="absolute left-0 top-0 w-[40%] h-full z-30 flex items-center justify-center">
      <div class="bg-[#2b2b2e] bg-opacity-70 rounded-xl p-8 w-[75%] max-w-3xl h-[65%] max-h-3xl text-white shadow-lg">
        <h2 class="text-3xl font-bold mb-2">Signup</h2>
        <p class="text-sm mb-6 text-gray-300">Just some details to get you in !</p>
        <form method="POST" action="{{ route('register') }}">
          @csrf
          <div class="mb-3">
          <input type="text" name="name" placeholder="{{ __('Name') }}" class="w-full p-2 mb-1 rounded bg-gray-800 border border-gray-600 focus:outline-none @error('name') is-invalid @enderror" required autocomplete="name" autofocus>
          ...
          <div class="mb-3">
          <input type="email" name="email" placeholder="{{ __('Email') }}" class="w-full p-2 mb-1 rounded bg-gray-800 border border-gray-600 focus:outline-none @error('email') is-invalid @enderror" required autocomplete="email">
          <div class="input-group-append">
                  <div class="input-group-text">
                      <span class="fas fa-user"></span>
                  </div>
          ...
          <div class="password-container mb-3">
            <input type="password" name="password" id="password" placeholder="{{ __('Password') }}" class="w-full p-2 pr-10 rounded bg-gray-800 border border-gray-600 focus:outline-none @error('password') is-invalid @enderror" required autocomplete="new-password">
          ...
          <div class="password-container mb-4">
            <input type="password" id="confirmPassword" name="password_confirmation" placeholder="{{ __('Confirm Password') }}" class="w-full p-2 pr-10 rounded bg-gray-800 border border-gray-600 focus:outline-none is-invalid @error('password_confirmation') @enderror" required autocomplete="new-password">
          ...
          <button type="submit" class="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white py-2 rounded font-semibold">{{ __('Register') }}</button>
        </form>
