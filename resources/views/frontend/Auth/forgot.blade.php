  ...
  <body class="h-screen m-0 p-0 overflow-hidden relative">
    ...
    <!-- Login Form -->
        ...
        <div class="border-4 border-[#2b2b2e] rounded-xl p-8 w-[75%] max-w-3xl h-[40%] max-h-3xl text-white shadow-lg flex flex-col justify-center">
            ...
            @endif
            <form method="POST" action="{{ route('password.email') }}" class="flex-grow flex flex-col justify-center @error('email') is-invalid @enderror">
              @csrf
            <div>
              <input type="email" name="email" placeholder="{{ __('Email') }}" class="w-full p-2 mb-4 rounded bg-gray-800 border border-gray-600 focus:outline-none" />
            ...
            <div>
                <button type="submit" class="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white py-2 rounded font-semibold">{{ __('Send Password Reset Link') }}</button>
