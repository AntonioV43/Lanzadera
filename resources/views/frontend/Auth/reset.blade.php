  ...
  <body class="h-screen m-0 p-0 overflow-hidden relative">
    ...
    <div class="absolute left-0 top-0 w-[40%] h-full z-30 flex items-center justify-center">
      <div class="border-4 border-[#2b2b2e] rounded-xl p-8 w-[75%] max-w-3xl h-[40%] max-h-3xl text-white shadow-lg flex flex-col justify-center">
        ...
        <form method="POST" action="{{ route('password.update') }}" class="flex-grow flex flex-col justify-center">
          @csrf
          <input type="hidden" name="token" value="{{ $token }}">
          
          <div>
            <input type="email" name="email" placeholder="E-mail" class="w-full p-2 mb-4 rounded bg-gray-800 border border-gray-600 focus:outline-none @error('email') is-invalid @enderror" required autofocus>
          ...
          <div class="password-container mb-4">
            <input type="password" name="password" id="password" placeholder="{{ __('Password') }}" class="w-full p-2 pr-10 rounded bg-gray-800 border border-gray-600 focus:outline-none @error('password') is-invalid @enderror" required>
          ...
          <div class="password-container mb-4">
            <input type="password" name="password_confirmation" id="confirmPassword" placeholder="{{ __('Confirm Password') }}s" class="w-full p-2 pr-10 rounded bg-gray-800 border border-gray-600 focus:outline-none @error('password_confirmation') is-invalid @enderror" required>
          ...
          <div>
            <button type="submit" class="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white py-2 rounded font-semibold">{{ __('Reset Password') }}</button>
          </div>
        </form>
      </div>
    </div>
